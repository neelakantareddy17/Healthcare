...
export const createAppointment = async (input: CreateAppointmentInput) => {
  ...
  if (appointmentDay.getTime() < startOfDay(new Date()).getTime()) {
    throw ApiError.badRequest('Cannot book an appointment in the past');
  }

  await assertDoctorNotOnLeave(input.doctorId, appointmentDay);
...
export const getAppointments = async (filters: AppointmentFilters) => {
  ...
  if (filters.date) {
    where.appointmentDate = { gte: startOfDay(filters.date), lte: endOfDay(filters.date) };
  }
...
export const cancelAppointment = async (id: string, requesterUserId: string, isAdmin: boolean) => {
  const appointment = await getAppointmentById(id);

  if (!isAdmin && appointment.patient.userId !== requesterUserId) {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    // Free up the queue slot / mark payment refunded if applicable
