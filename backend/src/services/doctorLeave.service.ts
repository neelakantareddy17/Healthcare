...
export const createLeave = async (input: CreateLeaveInput) => {
  ...
  // Prevent leave overlapping with existing confirmed (paid/checked-in) appointments
  const conflicting = await prisma.appointment.count({
    where: {
      doctorId: input.doctorId,
      appointmentDate: { gte: startDate, lte: endDate },
      status: { in: ['PENDING', 'PAID', 'CHECKED_IN', 'IN_PROGRESS'] },
    },
  });
...
export const getLeavesForDoctor = async (doctorId: string) => {
...
export const deleteLeave = async (id: string, doctorId: string, isAdmin: boolean) => {
  const leave = await getLeaveById(id);
  if (!isAdmin && leave.doctorId !== doctorId) {
    throw ApiError.forbidden('You can only remove your own leave records');
  }
  await prisma.doctorLeave.delete({ where: { id } });
  return { message: 'Leave record removed' };
};
